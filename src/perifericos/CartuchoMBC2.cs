...
namespace csgbe.perifericos{
	...
	/// <summary>Cartucho MBC2 (solo 1 banco de RAM)</summary>
	public class CartuchoMBC2 : Cartucho{

		/// <summary>Definicion del tipo de cartucho</summary>
		private static string tipoCartucho = "MBC2";
		...
		public CartuchoMBC2(string nombreFichero) : base(nombreFichero){}
		...
		/// <returns>Byte posicionado en la direccion</returns>
			...
			if (direccion >= 0){
				...
				// Banco 0 de RAM
				else if (direccion >= 0xA000 && direccion < 0xC000 && _ramEnabled) valor = _ram[(direccion - 0xA000)];
			}
			return valor;
		}
		...
		public override void escribir(int valor, int direccion){
			...
			else if (direccion < 0x4000) _romPage = valor & 0x0F; 
			// RAM
			else if (direccion >= 0xA000 && direccion < 0xC000 && _ramEnabled) _ram[(direccion - 0xA000)] = (byte)valor;
		}
	}
}
